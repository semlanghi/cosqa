...
public class DummyRepairStock {

    public static void main(String[] args) {


        Logger logger = LoggerFactory.getLogger(DummyRepairStock.class);
...
//        props.put(StreamsConfig.topicPrefix(TopicConfig.RETENTION_MS_CONFIG), Duration.ofDays(10).toMillis());
        ...
        props.put(ExperimentConfig.RESULT_FILE_SUFFIX, "dummy-songrepair-stock");
        ...
        builder.stream(topic, Consumed.with(Serdes.String(), StockSerde.instance(), new TimestampExtractor() {
                            @Override
                            public long extract(ConsumerRecord<Object, Object> record, long partitionTime) {
                                return ((Stock)record.value()).getTs();
                            }
                        }, Topology.AutoOffsetReset.EARLIEST))
                        .transform(new TransformerSupplier<String, Stock, KeyValue<String, Stock>>() {
                            @Override
...
//                .process(new ProcessorSupplier<String, Stock, Void, Void>() {
//                    @Override
