...
public class NCOSQASchemaConstraintDummyGPS {

    public static void main(String[] args) {

        Logger logger = LoggerFactory.getLogger(NCOSQASchemaConstraintDummyGPS.class);
...
//        props.put(StreamsConfig.topicPrefix(TopicConfig.RETENTION_MS_CONFIG), Duration.ofDays(10).toMillis());
        ...
        props.put(ExperimentConfig.RESULT_FILE_SUFFIX, "dummy-ncosqa-gps-schema");
...
//        AnnKStream<String, GPS> annotatedKStream = AnnKStream.annotateGraphListNotWindowed(builder
...
//
//
        AnnKStream<String, GPS> annotatedKStream = AnnKStream.annotateSchemaNotWindowed(builder
                    .stream(topic, Consumed.with(Serdes.String(), GPSSerde.instance(), new TimestampExtractor() {
                        @Override
                        public long extract(ConsumerRecord<Object, Object> record, long partitionTime) {
                            return ((GPS)record.value()).timestamp();
                        }
                    }, Topology.AutoOffsetReset.EARLIEST)), new SchemaConstraintGPSValueFactory(), applicationSupplier, props);
