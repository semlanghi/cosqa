...
public class ExampleSong {
    ...
    public static void main(String[] args) {
        ...
        Logger logger = LoggerFactory.getLogger(ExampleSong.class);
        ...
        annotatedKStream
                .mapValuesOnAnnotation(new ValueMapper<ConsistencyAnnotatedRecord<ValueAndTimestamp<Stock>>, Stock>() {
                    ...
                    public Stock apply(ConsistencyAnnotatedRecord<ValueAndTimestamp<Stock>> value) {
                        ...
                        if (maxExpon>value.getWrappedRecord().value().getValue())
                            adjustedValue = value.getWrappedRecord().value().getValue() - (maxExpon/2.0);
                        else adjustedValue = value.getWrappedRecord().value().getValue() - maxExpon;
