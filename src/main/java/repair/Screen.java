...
public class Screen<V> extends ValueRepair<V> {

  private double smin;
  private double smax;
  private double w;

  public Screen(Iterator<ValueAndTimestamp<V>> dataIterator, int... opt) throws Exception {
    super(dataIterator, opt);
    setParameters();
  }

  private void setParameters() {
    // set the default speed threshold
    double[] speed = repair.Util.speed(original, time);
    Median median = new Median();
    double mid = median.evaluate(speed);
    double sigma = repair.Util.mad(speed);
    smax = mid + 3 * sigma;
    smin = mid - 3 * sigma;
    // set the default window size
    w = time[time.length-1]-time[0];
