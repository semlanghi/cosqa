...
public class TopKCAProcessorNI<K,V> extends ContextualProcessor<Windowed<K>, V, Void, Void> {
    ...
    public TopKCAProcessorNI(String topKStoreName, ApplicationSupplier shutdownHook, Properties properties) {
    ...
    public void process(Record<Windowed<K>, V> record) {
        ...
        KeyValueIterator<Windowed<K>, V> range = topKStore.range(record.key(), record.key());
...
//        }
    ...
    private void registerPerf(Record<Windowed<K>, V> record) {
        try {
