...
public class TopKCAProcessor<K,V> extends ContextualProcessor<Windowed<K>, ConsistencyAnnotatedRecord<ValueAndTimestamp<V>>, Void, Void> {


    private KeyValueStore<Windowed<K>, ConsistencyAnnotatedRecord<ValueAndTimestamp<V>>> topKStore;
    ...
    public TopKCAProcessor(String topKStoreName, ApplicationSupplier shutdownHook, Properties properties) {
    ...
    @Override
    public void process(Record<Windowed<K>, ConsistencyAnnotatedRecord<ValueAndTimestamp<V>>> record) {
        topKStore.put(record.key(), record.value());
        KeyValueIterator<Windowed<K>, ConsistencyAnnotatedRecord<ValueAndTimestamp<V>>> range = topKStore.range(record.key(), record.key());

//        int rank = 1;
//        while (range.hasNext()){
//            //forwardElement
//            KeyValue<Windowed<K>, ConsistencyAnnotatedRecord<ValueAndTimestamp<V>>> next = range.next();
////            context().forward(new Record<>(next.key, rank, context().currentStreamTimeMs()));
//            System.out.println(new Record<>(next.key, rank , context().currentStreamTimeMs()) + " ic: "+ next.value.getPolynomial().toString());
//            rank++;
//        }
...
//        registerPerf(record);
    ...
    private void registerPerf(Record<Windowed<K>, ConsistencyAnnotatedRecord<ValueAndTimestamp<V>>> record) {
        try {
