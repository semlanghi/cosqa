...
public class TopKCAProcessorNotWindowed<K,V> extends ContextualProcessor<K, ConsistencyAnnotatedRecord<V>, Void, Void> {
    ...
    public TopKCAProcessorNotWindowed(String topKStoreName, AnnotationAwareTimeWindows timeWindows, ApplicationSupplier shutdownHook, Properties properties) {
    ...
    @Override
    public void process(Record<K, ConsistencyAnnotatedRecord<V>> record) {
        ...
        KeyValueIterator<Windowed<K>, ConsistencyAnnotatedRecord<V>> range = topKStore.range(key, key);
...
//        }
    ...
    private void registerPerf(Record<K,ConsistencyAnnotatedRecord<V>> record) {
        try {
