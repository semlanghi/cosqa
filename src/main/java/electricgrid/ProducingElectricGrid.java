...
public class ProducingElectricGrid {
    public static void main(String[] args){
        ...
        try {
            ...
            producer.send(new ProducerRecord<>(topic, 1, new EGC(1, 50, 20, 5L, UUID.randomUUID())));
            producer.send(new ProducerRecord<>(topic, 1, new EGC(1, 20, 20, 6L, UUID.randomUUID())));
            producer.send(new ProducerRecord<>(topic, 1, new EGC(1, 0, 20, 7L, UUID.randomUUID())));
            producer.send(new ProducerRecord<>(topic, 1, new EGC(1, 20, 20, 8L, UUID.randomUUID())));
            producer.send(new ProducerRecord<>(topic, 1, new EGC(1, 40, 20, 9L, UUID.randomUUID())));
            producer.send(new ProducerRecord<>(topic, 1, new EGC(1, 60, 20, 10L, UUID.randomUUID())));
