        ...
        for ws in "${window_size[@]}"; do
            ...
            for wssf in "${window_size_slide_factor[@]}"; do
                ...
                for ((j=1; j<=3; j++)); do
                    echo "Iteration $j"
                    topic="reviews-nrecords-$3"
                    java -Xmx"$MEMORY" -cp ../target/COSQA-jar-with-dependencies.jar "${name_experiment[$i]}"Review "$cs" 0 "$((ws*86400000))" $(((ws/wssf)*86400000)) "$1" "$(($2/2))" $(($2*2)) "$topic" &> ${name_experiment[$i]}-review.out &
                    wait
                    topic="gps-nrecords-$3"
                    java -Xmx"$MEMORY" -cp ../target/COSQA-jar-with-dependencies.jar "${name_experiment[$i]}"GPS "$cs" 0 "$ws" $((ws/wssf)) "$1" "$(($2/2))" $(($2*2)) "$topic" &> ${name_experiment[$i]}-gps.out &
                    wait
                    topic="stocks-nrecords-$3"
                    java -Xmx"$MEMORY" -cp ../target/COSQA-jar-with-dependencies.jar "${name_experiment[$i]}"StockCombo "$cs" 0 "$((ws*86400000))" $(((ws/wssf)*86400000)) "$1" "$(($2/2))" $(($2*2)) "$topic" &> ${name_experiment[$i]}-stock-combo.out &
                    wait
                    topic="stocks-nrecords-$3"
                    java -Xmx"$MEMORY" -cp ../target/COSQA-jar-with-dependencies.jar "${name_experiment[$i]}"StockPearson "$cs" 0 "$((ws*86400000))" $(((ws/wssf)*86400000)) "$1" "$(($2/5))" $(($2*2)) "$topic" &> ${name_experiment[$i]}-stock-pearson.out &
                    wait
